...
def append_name ():
    ...
    #setup random receipt
    def random_receipt():
        receipt_number = random.randint(1,10000)
    customer_details.append([entry_customer_name.get(),item.get(),entry_amount.get(),random_receipt])
    #clear the boxes
...
def button_setup():
    ...
    Button(main_window,
           ...
           font=("Bahnschrift", 12, "bold"),
           command=delete_row,
           width=12).grid(column=8, row=8, pady=5, columnspan=8, sticky=E)
...
delete_item.grid(column=3,row=8)    
main()
