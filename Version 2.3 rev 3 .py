...
def print_customer_details ():
    ...
    #Create column headings
        Label(main_window, font=("Bahnschrift", 10, "bold"),text="Reciept Number").grid(column=0,row=7,pady=25)
        Label(main_window, font=("Bahnschrift", 10, "bold"),text="Full Name").grid(column=1,row=7)
        Label(main_window, font=("Bahnschrift", 10, "bold"),text="Item Hired ").grid(column=2,row=7)
        Label(main_window, font=("Bahnschrift", 10, "bold"),text="Amount Hired").grid(column=3,row=7)
...
def check_inputs ():
    input_check = 0 #program assumes that all inputs are invalid
    ...
    input_check = 1 #this variable assumes all the details are correct.
    #if input_check == 1 : append_entry()
...
def append_entry ():
    ...
    customer_details.append([receipt_num,customer_name.get(),item.get(),entry_amount.get()])
    #clear the boxes
...
def saving_details():
    with open("customerdeats.txt","a") as file:
        cname = customer_name.get() + " " + item.get()+ " " +entry_amount.get()
        file.write(customer_name.get())
        file.write(" "+ item.get())
        file.write(" "+ str(entry_amount.get())+"\n")
        print("name: "+cname)
        file.close()
        append_entry()
...
def button_setup():
    ...
    Button(main_window,text="Submit",font=("Bahnschrift", 12, "bold"),command=lambda: [check_inputs(),saving_details()],width=12).grid(column=2,row=1,pady=10,sticky=E)
